...
public class Client {
	...
	private void createMietspiegel(){
		try {
			...
			conn.createStatement().executeUpdate("insert into mietspiegel (ort,miete) values ('Berlin', 1200)");
	...
	private void createKindergarten() {
		try {
			...
			conn.createStatement().executeUpdate("insert into kindergarten (ort,beitrag) values ('Berlin', 100)");
		} catch (SQLException e) {
			e.printStackTrace();
		}
	...
	public static void main(String[] args) throws XAException, SQLException {
		...
		c1.res.start(xid1, 0);
		c2.res.start(xid2, 0);
		int i1 = c1.conn.createStatement().executeUpdate("insert into mietspiegel (ort,miete) values ('Wallenfels', 300)");
		int i2 = c2.conn.createStatement().executeUpdate("insert into kindergarten (ort,beitrag) values ('Wallenfels', 20)");
